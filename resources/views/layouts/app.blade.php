...
<body class="font-['Plus_Jakarta_Sans'] antialiased h-full text-slate-900 dark:text-slate-100 bg-slate-50 dark:bg-slate-950 transition-colors duration-300 selection:bg-indigo-500 selection:text-white">
    ...
    <div x-data="{ sidebarOpen: true }" class="min-h-screen flex flex-col bg-slate-50 dark:bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] dark:from-slate-900 dark:via-slate-950 dark:to-black">
        ...
        <aside class="fixed inset-y-0 left-0 z-50 hidden flex-col border-r border-slate-200/80 bg-white/90 px-3 py-5 shadow-2xl shadow-slate-950/5 backdrop-blur-2xl transition-all duration-300 dark:border-slate-800/80 dark:bg-slate-950/90 lg:flex" :class="sidebarOpen ? 'w-72' : 'w-20'">
            <div class="flex items-center justify-between px-3">
                <a href="{{ route('home') }}" wire:navigate class="flex items-center gap-3 overflow-hidden">
                    ...
                    <div x-show="sidebarOpen" x-transition class="whitespace-nowrap">
                        ...
                        <span class="block text-[9px] font-bold uppercase tracking-[0.2em] text-indigo-500">Mutualisation</span>
                    </div>
                </a>
                <button @click="sidebarOpen = !sidebarOpen" type="button" class="rounded-xl p-2 text-slate-500 transition hover:bg-slate-100 hover:text-indigo-500 dark:hover:bg-slate-800" aria-label="Rétracter la sidebar">
            ...
            <nav class="mt-10 flex-1 space-y-2">
                <a href="{{ route('home') }}" wire:navigate class="flex items-center gap-3 rounded-2xl px-3 py-3 text-sm font-bold text-slate-600 transition hover:bg-indigo-50 hover:text-indigo-600 dark:text-slate-400 dark:hover:bg-indigo-950/40 dark:hover:text-indigo-300">
                ...
                @auth
                    ...
                    @if(auth()->user()->isAdmin() || auth()->user()->isTopManagement() || auth()->user()->isResponsableFinancier() || auth()->user()->isResponsableRh() || auth()->user()->isChefProjet())
                        ...
                        @if(auth()->user()->isAdmin() || auth()->user()->isTopManagement() || auth()->user()->isResponsableFinancier() || auth()->user()->isResponsableRh() || auth()->user()->isChefProjet())
                            <a href="{{ route('admin.contributions') }}" wire:navigate class="flex items-center gap-3 rounded-2xl px-3 py-3 text-sm font-bold text-slate-600 transition hover:bg-emerald-50 hover:text-emerald-600 dark:text-slate-400 dark:hover:bg-emerald-950/40 dark:hover:text-emerald-300">
                        ...
                        @if(auth()->user()->isAdmin() || auth()->user()->isTopManagement() || auth()->user()->isChefProjet())
                            <a href="{{ route('admin.projects.review') }}" wire:navigate class="flex items-center gap-3 rounded-2xl px-3 py-3 text-sm font-bold text-slate-600 transition hover:bg-indigo-50 hover:text-indigo-600 dark:text-slate-400 dark:hover:bg-indigo-950/40 dark:hover:text-indigo-300">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex items-center justify-between h-20">
                    ...
                    <div class="flex items-center gap-4">
                        <a href="{{ Route::has('home') ? route('home') : '/' }}" class="flex items-center gap-3 cursor-pointer select-none group" wire:navigate>
                            ...
                            <div>
                                <span class="text-xl font-extrabold tracking-tight text-slate-900 dark:text-white group-hover:text-indigo-500 transition-colors">MUTUALIS</span>
