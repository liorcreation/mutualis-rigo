...
<section class="space-y-4">
    <header>
        <h2 class="text-lg font-black text-rose-600 dark:text-rose-300">
    ...
    <button
        ...
        class="rounded-2xl bg-rose-50 dark:bg-rose-500/15 px-6 py-3 text-xs font-black uppercase tracking-widest text-rose-600 dark:text-rose-300 transition hover:bg-rose-500 hover:text-white"
    ...
    <x-ui.modal wireProperty="confirmingDeletion" onClose="$set('confirmingDeletion', false)" title="Êtes-vous sûr de vouloir supprimer votre compte ?">
        <form wire:submit="deleteUser" class="space-y-5">
            ...
            <div class="flex justify-end gap-3">
                <button type="button" wire:click="$set('confirmingDeletion', false)" class="rounded-2xl border border-slate-200 dark:border-white/10 px-5 py-3 text-xs font-bold text-slate-600 dark:text-slate-300 transition hover:bg-slate-100 dark:hover:bg-white/10 hover:text-slate-900 dark:hover:text-white">
                    {{ __('Annuler') }}
                </button>
