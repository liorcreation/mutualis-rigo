<x-ui.modal wireProperty="isOpen" onClose="close" title="Proposer un apport">
    <x-slot:header>
        <p class="text-[10px] font-bold uppercase tracking-[0.2em] text-indigo-600 dark:text-indigo-300">Contribution rapide</p>
        <h2 class="mt-2 text-2xl font-black tracking-tight text-slate-900 dark:text-white">Faites avancer ce projet</h2>
        <p class="mt-2 text-sm text-slate-500 dark:text-slate-400">Votre apport sera vérifié avant d'être mutualisé.</p>
    </x-slot:header>

    <form wire:submit="save" class="space-y-5">
        <div class="grid grid-cols-3 gap-3">
            <button wire:click="$set('typeApport', 'financier')" type="button" class="rounded-2xl border px-3 py-4 text-left transition {{ $typeApport === 'financier' ? 'border-emerald-300 dark:border-emerald-400/50 bg-emerald-50 dark:bg-emerald-400/10 text-emerald-700 dark:text-emerald-200 shadow-lg shadow-emerald-100 dark:shadow-emerald-950/30' : 'border-slate-200 dark:border-white/10 bg-white dark:bg-white/[0.04] text-slate-500 dark:text-slate-400 hover:border-slate-300 dark:hover:border-white/20 hover:text-slate-900 dark:hover:text-white' }}">
            ...
            <button wire:click="$set('typeApport', 'competence')" type="button" class="rounded-2xl border px-3 py-4 text-left transition {{ $typeApport === 'competence' ? 'border-fuchsia-300 dark:border-fuchsia-400/50 bg-fuchsia-50 dark:bg-fuchsia-400/10 text-fuchsia-700 dark:text-fuchsia-200 shadow-lg shadow-fuchsia-100 dark:shadow-fuchsia-950/30' : 'border-slate-200 dark:border-white/10 bg-white dark:bg-white/[0.04] text-slate-500 dark:text-slate-400 hover:border-slate-300 dark:hover:border-white/20 hover:text-slate-900 dark:hover:text-white' }}">
            ...
            <button wire:click="$set('typeApport', 'materiel')" type="button" class="rounded-2xl border px-3 py-4 text-left transition {{ $typeApport === 'materiel' ? 'border-amber-300 dark:border-amber-400/50 bg-amber-50 dark:bg-amber-400/10 text-amber-700 dark:text-amber-200 shadow-lg shadow-amber-100 dark:shadow-amber-950/30' : 'border-slate-200 dark:border-white/10 bg-white dark:bg-white/[0.04] text-slate-500 dark:text-slate-400 hover:border-slate-300 dark:hover:border-white/20 hover:text-slate-900 dark:hover:text-white' }}">
        ...
        @error('typeApport') <p class="text-xs font-medium text-rose-500 dark:text-rose-400">{{ $message }}</p> @enderror

        <div class="rounded-2xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-black/10 px-4 py-3">
            <p class="text-[10px] font-bold uppercase tracking-wider text-slate-500">Projet sélectionné</p>
            <p class="mt-1 text-sm font-bold text-slate-700 dark:text-slate-200">{{ $projectId ? 'Projet #'.$projectId : 'Sélectionnez un projet depuis le catalogue' }}</p>
            @error('projectId') <p class="mt-1 text-xs font-medium text-rose-500 dark:text-rose-400">{{ $message }}</p> @enderror
        </div>

        @if($typeApport === 'financier')
            <label class="block">
                <span class="mb-2 block text-xs font-bold text-slate-700 dark:text-slate-300">Montant proposé <span class="text-rose-500 dark:text-rose-400">*</span></span>
                <div class="relative">
                    <input wire:model.live.debounce.250ms="montant" type="number" min="1" step="0.01" placeholder="Ex. 150000" class="w-full rounded-2xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-950/70 px-4 py-3.5 pr-16 text-sm text-slate-900 dark:text-white outline-none transition placeholder:text-slate-400 dark:placeholder:text-slate-600 focus:border-emerald-400/60 focus:ring-4 focus:ring-emerald-500/10 @error('montant') border-rose-400/70 @enderror">
