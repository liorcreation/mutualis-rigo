<div class="min-h-screen bg-slate-950 text-slate-100">
    <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:py-12">
        @if(session('reservation-message'))<div class="mb-6 rounded-2xl border border-emerald-400/20 bg-emerald-400/10 px-4 py-3 text-sm font-semibold text-emerald-200">{{ session('reservation-message') }}</div>@endif
        ...
        <div class="relative overflow-hidden rounded-3xl border border-white/10 bg-white/[0.055] p-6 shadow-2xl shadow-black/20 backdrop-blur-2xl sm:p-8">
            <div class="pointer-events-none absolute -right-20 -top-32 h-72 w-72 rounded-full bg-amber-500/15 blur-3xl"></div>
            <div class="relative">
                <p class="text-[10px] font-black uppercase tracking-[0.2em] text-amber-300">Mutualisation matérielle</p>
        ...
        <div class="mt-10 flex gap-2 overflow-x-auto border-b border-white/10">
            <button wire:click="selectTab('catalogue')" type="button" class="whitespace-nowrap border-b-2 px-4 py-4 text-xs font-black transition {{ $activeTab === 'catalogue' ? 'border-amber-400 text-amber-300' : 'border-transparent text-slate-500 hover:text-white' }}">Matériel disponible</button>
            <button wire:click="selectTab('mes-demandes')" type="button" class="whitespace-nowrap border-b-2 px-4 py-4 text-xs font-black transition {{ $activeTab === 'mes-demandes' ? 'border-amber-400 text-amber-300' : 'border-transparent text-slate-500 hover:text-white' }}">Mes demandes</button>
            <button wire:click="selectTab('demandes-recues')" type="button" class="whitespace-nowrap border-b-2 px-4 py-4 text-xs font-black transition {{ $activeTab === 'demandes-recues' ? 'border-amber-400 text-amber-300' : 'border-transparent text-slate-500 hover:text-white' }}">Demandes reçues</button>
        </div>
        ...
        <div class="mt-6" wire:loading.class="opacity-50" wire:target="selectTab,gotoPage,nextPage,previousPage">
                ...
                @if($availableMaterials->isNotEmpty())
                    <div class="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
                        @foreach($availableMaterials as $material)
                            <article wire:key="material-{{ $material->id }}" class="flex flex-col justify-between rounded-3xl border border-white/10 bg-white/[0.045] p-5 transition hover:-translate-y-1 hover:border-amber-400/30">
                                <div>
                                ...
                                <button wire:click="openRequestForm({{ $material->id }})" type="button" class="mt-5 w-full rounded-xl bg-amber-500/15 px-3 py-2.5 text-[11px] font-black text-amber-300 transition hover:bg-amber-500 hover:text-white">Demander une réservation</button>
                            </article>
                        @endforeach
                    </div>
                @else
                    <div class="rounded-3xl border border-dashed border-white/15 px-6 py-16 text-center">
                        <p class="text-sm font-bold text-white">Aucun matériel disponible pour l'instant</p>
                        <p class="mt-2 text-xs text-slate-500">Les apports matériels validés d'autres projets apparaîtront ici.</p>
                    </div>
                @endif
            ...
            @if($activeTab === 'mes-demandes')
                            ...
                            @foreach($myRequests as $reservation)
                                @php
                                    $badgeClass = match($reservation->statut) {
                                        \App\Enums\ReservationStatus::VALIDEE => 'border-emerald-400/20 bg-emerald-400/10 text-emerald-300',
                                        \App\Enums\ReservationStatus::REFUSEE => 'border-rose-400/20 bg-rose-400/10 text-rose-300',
                                        \App\Enums\ReservationStatus::ANNULEE => 'border-slate-400/20 bg-slate-400/10 text-slate-400',
                                ...
                                <div wire:key="my-reservation-{{ $reservation->id }}" class="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
                                    ...
                                    <div class="flex flex-wrap items-center gap-2">
                                        <span class="w-fit rounded-full border px-3 py-1 text-[10px] font-bold uppercase tracking-wider {{ $badgeClass }}">{{ ucfirst($reservation->statut->value) }}</span>
                ...
                @else
                    <div class="rounded-3xl border border-dashed border-white/15 px-6 py-16 text-center">
                        <p class="text-sm font-bold text-white">Aucune demande envoyée</p>
                        <p class="mt-2 text-xs text-slate-500">Parcourez le matériel disponible pour envoyer votre première demande.</p>
                    </div>
                @endif
            ...
            @if($activeTab === 'demandes-recues')
                            ...
                            @foreach($receivedRequests as $reservation)
                                <div wire:key="received-reservation-{{ $reservation->id }}" class="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
                                    ...
                                    <div class="flex flex-wrap items-center gap-2">
                                        ...
                                        @else
                                            <span class="w-fit rounded-full border px-3 py-1 text-[10px] font-bold uppercase tracking-wider {{ $reservation->statut === \App\Enums\ReservationStatus::VALIDEE ? 'border-emerald-400/20 bg-emerald-400/10 text-emerald-300' : 'border-rose-400/20 bg-rose-400/10 text-rose-300' }}">{{ ucfirst($reservation->statut->value) }}</span>
                                        @endif
                                    </div>
                                </div>
                            @endforeach
                ...
                @else
                    <div class="rounded-3xl border border-dashed border-white/15 px-6 py-16 text-center">
                        <p class="text-sm font-bold text-white">Aucune demande reçue</p>
                        <p class="mt-2 text-xs text-slate-500">Les demandes sur votre matériel mutualisé apparaîtront ici.</p>
                    </div>
                @endif
    ...
    @if($showRequestForm)
        <div class="fixed inset-0 z-[70] flex items-end justify-center bg-slate-950/75 p-0 backdrop-blur-sm sm:items-center sm:p-6">
            <button type="button" wire:click="closeRequestForm" class="absolute inset-0 cursor-default" aria-label="Fermer"></button>
            <section class="relative w-full max-w-lg overflow-hidden rounded-t-[2rem] border border-white/10 bg-slate-900 shadow-2xl shadow-black/50 sm:rounded-3xl">
                ...
                <div class="flex items-start justify-between px-6 pb-5 pt-6 sm:px-8 sm:pt-8">
                    <div>
                        <p class="text-[10px] font-bold uppercase tracking-[0.2em] text-amber-300">Demande de réservation</p>
                    ...
                    <button type="button" wire:click="closeRequestForm" class="rounded-xl border border-white/10 p-2 text-slate-400 transition hover:bg-white/10 hover:text-white" aria-label="Fermer">
                ...
                <form wire:submit="submitRequest" class="space-y-5 px-6 pb-7 sm:px-8 sm:pb-8">
                    <div class="grid grid-cols-2 gap-3">
                        <label class="block">
                            <span class="mb-2 block text-xs font-bold text-slate-300">Du <span class="text-rose-400">*</span></span>
                            <input wire:model.live="dateDebut" type="date" class="w-full rounded-2xl border border-white/10 bg-slate-950/70 px-4 py-3 text-sm text-white outline-none transition focus:border-amber-400/60 focus:ring-4 focus:ring-amber-500/10 @error('dateDebut') border-rose-400/70 @enderror">
                        ...
                        <label class="block">
                            ...
                            <input wire:model.live="dateFin" type="date" class="w-full rounded-2xl border border-white/10 bg-slate-950/70 px-4 py-3 text-sm text-white outline-none transition focus:border-amber-400/60 focus:ring-4 focus:ring-amber-500/10 @error('dateFin') border-rose-400/70 @enderror">
                    ...
                    <label class="block">
                        ...
                        <input wire:model.live.debounce.250ms="quantite" type="number" min="1" step="1" class="w-full rounded-2xl border border-white/10 bg-slate-950/70 px-4 py-3.5 text-sm text-white outline-none transition focus:border-amber-400/60 focus:ring-4 focus:ring-amber-500/10 @error('quantite') border-rose-400/70 @enderror">
    ...
    @if($showReview)
        <div class="fixed inset-0 z-[70] flex items-end justify-center bg-slate-950/75 p-0 backdrop-blur-sm sm:items-center sm:p-6">
            <button wire:click="closeReview" type="button" class="absolute inset-0 cursor-default" aria-label="Fermer"></button>
            <div class="relative w-full max-w-lg rounded-t-3xl border border-white/10 bg-slate-900 p-6 shadow-2xl sm:rounded-3xl sm:p-8">
                <div class="absolute inset-x-0 top-0 h-1 {{ $decision === 'validee' ? 'bg-emerald-400' : 'bg-rose-400' }}"></div>
                <div class="flex items-start justify-between">
                    ...
                    <button wire:click="closeReview" type="button" class="rounded-xl p-2 text-slate-500 hover:bg-white/10 hover:text-white">×</button>
                </div>
                <form wire:submit="saveDecision" class="mt-6 space-y-5">
                    <label class="block">
                        ...
                        <textarea wire:model.live.debounce.250ms="commentaireValidation" rows="4" placeholder="Expliquez votre décision au demandeur..." class="w-full resize-none rounded-2xl border border-white/10 bg-slate-950/70 px-4 py-3.5 text-sm text-white outline-none focus:border-amber-400/60 focus:ring-4 focus:ring-amber-500/10 @error('commentaireValidation') border-rose-400/70 @enderror"></textarea>
