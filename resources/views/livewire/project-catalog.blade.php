<div class="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100">
    @if(session('project-message'))
        <div class="mx-auto mt-6 max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="rounded-2xl border border-emerald-200 dark:border-emerald-400/20 bg-emerald-50 dark:bg-emerald-400/10 px-4 py-3 text-sm font-semibold text-emerald-700 dark:text-emerald-200">{{ session('project-message') }}</div>
        </div>
    @endif
    <div class="relative isolate overflow-hidden">
        <div class="pointer-events-none absolute -left-40 top-0 -z-10 h-96 w-96 rounded-full bg-indigo-400/10 dark:bg-indigo-600/20 blur-3xl"></div>
        <div class="pointer-events-none absolute right-0 top-40 -z-10 h-80 w-80 rounded-full bg-fuchsia-400/5 dark:bg-fuchsia-600/10 blur-3xl"></div>

        <section class="mx-auto max-w-7xl px-4 pb-8 pt-12 sm:px-6 lg:px-8 lg:pt-16">
            <div class="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
                <div class="max-w-3xl">
                    <div class="mb-5 inline-flex items-center gap-2 rounded-full border border-indigo-200 dark:border-indigo-400/20 bg-indigo-50 dark:bg-indigo-400/10 px-3 py-1.5 text-[11px] font-bold uppercase tracking-[0.2em] text-indigo-600 dark:text-indigo-300">
                    ...
                    <h1 class="text-4xl font-black tracking-tight text-slate-900 dark:text-white sm:text-5xl">
                        ...
                        <span class="bg-gradient-to-r from-indigo-600 via-fuchsia-600 to-emerald-600 dark:from-indigo-300 dark:via-fuchsia-300 dark:to-emerald-300 bg-clip-text text-transparent">ensemble.</span>
                    </h1>
                    <p class="mt-5 max-w-2xl text-sm leading-7 text-slate-500 dark:text-slate-400 sm:text-base">
                ...
                <div class="grid grid-cols-2 gap-3 sm:flex sm:items-center">
                    <div class="rounded-2xl border border-slate-200/80 dark:border-white/10 bg-white/70 dark:bg-white/[0.06] px-5 py-4 backdrop-blur-xl">
                        <div class="text-2xl font-black text-slate-900 dark:text-white">{{ $projects->total() }}</div>
                        <div class="mt-1 text-[10px] font-bold uppercase tracking-widest text-slate-500">Projets visibles</div>
                    </div>
                    <div class="rounded-2xl border border-emerald-200 dark:border-emerald-400/20 bg-emerald-50 dark:bg-emerald-400/[0.06] px-5 py-4 backdrop-blur-xl">
                        <div class="text-2xl font-black text-emerald-600 dark:text-emerald-300">{{ $projects->currentPage() }}</div>
                        <div class="mt-1 text-[10px] font-bold uppercase tracking-widest text-emerald-600/70 dark:text-emerald-300/60">Page actuelle</div>
                    </div>
                </div>
            </div>
        </section>
        ...
        <section class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="rounded-3xl border border-slate-200/80 dark:border-white/10 bg-white/70 dark:bg-white/[0.055] p-4 shadow-sm dark:shadow-2xl dark:shadow-black/20 backdrop-blur-xl dark:backdrop-blur-2xl sm:p-5">
                <div class="flex flex-col gap-4 xl:flex-row xl:items-center">
                    <label class="group relative block flex-1">
                        <span class="pointer-events-none absolute inset-y-0 left-4 flex items-center text-slate-400 dark:text-slate-500 transition-colors group-focus-within:text-indigo-600 dark:group-focus-within:text-indigo-400">
                        ...
                        <input wire:model.live.debounce.350ms="search" type="search" placeholder="Rechercher un projet, une catégorie..." class="w-full rounded-2xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-950/60 py-3.5 pl-12 pr-4 text-sm text-slate-900 dark:text-white outline-none transition placeholder:text-slate-400 dark:placeholder:text-slate-600 focus:border-indigo-400/60 focus:ring-4 focus:ring-indigo-500/10">
                    </label>

                    <div class="grid grid-cols-2 gap-3 sm:flex">
                        <select wire:model.live="needType" class="rounded-2xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-950/60 px-4 py-3.5 text-sm text-slate-700 dark:text-slate-300 outline-none transition focus:border-indigo-400/60 focus:ring-4 focus:ring-indigo-500/10">
                            <option value="all">Tous les besoins</option>
                            <option value="financier">Financier</option>
                            <option value="competence">Compétences</option>
                            <option value="materiel">Matériel</option>
                        </select>
                        <select wire:model.live="status" class="rounded-2xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-950/60 px-4 py-3.5 text-sm text-slate-700 dark:text-slate-300 outline-none transition focus:border-indigo-400/60 focus:ring-4 focus:ring-indigo-500/10">
                ...
                @if($search !== '' || $needType !== 'all' || $status !== 'all')
                    <div class="mt-4 flex items-center justify-between border-t border-slate-200 dark:border-white/10 pt-4 text-xs">
                        <span class="text-slate-500">Filtres actifs · mise à jour instantanée</span>
                        <button wire:click="clearFilters" type="button" class="font-bold text-indigo-600 dark:text-indigo-300 transition hover:text-slate-900 dark:hover:text-white">Réinitialiser</button>
                    </div>
                @endif
            </div>
        </section>
        ...
        <section class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 lg:py-12">
            <div wire:loading.class="opacity-50" wire:target="search,needType,status,gotoPage,nextPage,previousPage" class="transition-opacity duration-200">
                            ...
                            <article wire:key="project-{{ $project->id }}" class="group relative flex min-h-[490px] flex-col overflow-hidden rounded-3xl border border-slate-200/80 dark:border-white/10 bg-white/70 dark:bg-white/[0.055] shadow-sm dark:shadow-xl dark:shadow-black/10 backdrop-blur-xl transition duration-300 hover:-translate-y-1 hover:border-indigo-300 dark:hover:border-indigo-400/40 hover:bg-white dark:hover:bg-white/[0.08] hover:shadow-indigo-950/10 dark:hover:shadow-indigo-950/40">
                                ...
                                <div class="flex-1 px-6 pb-5 pt-5">
                                    ...
                                    <div class="mt-5 flex flex-wrap gap-2">
                                        @foreach($skills as $skill)
                                            <span class="rounded-lg border border-fuchsia-200 dark:border-fuchsia-300/15 bg-fuchsia-50 dark:bg-fuchsia-400/10 px-2.5 py-1 text-[10px] font-semibold text-fuchsia-700 dark:text-fuchsia-200">{{ $skill }}</span>
                                        @endforeach
                                        @foreach($materials as $material)
                                            <span class="rounded-lg border border-amber-200 dark:border-amber-300/15 bg-amber-50 dark:bg-amber-400/10 px-2.5 py-1 text-[10px] font-semibold text-amber-700 dark:text-amber-200">{{ $material }}</span>
                                        @endforeach
                                        @if($project->besoin_financier_target > 0 || $project->budget > 0)
                                            <span class="rounded-lg border border-emerald-200 dark:border-emerald-300/15 bg-emerald-50 dark:bg-emerald-400/10 px-2.5 py-1 text-[10px] font-semibold text-emerald-700 dark:text-emerald-200">Financement</span>
                                        @endif
                                    </div>
                                    ...
                                    <div class="mt-7 space-y-5">
                                        <div>
                                            <div class="mb-2 flex items-center justify-between text-[11px] font-bold uppercase tracking-wider">
                                                <span class="flex items-center gap-2 text-emerald-600 dark:text-emerald-300"><span class="h-2 w-2 rounded-full bg-emerald-500 dark:bg-emerald-400"></span>Progression financière</span>
                                                <span class="text-slate-900 dark:text-white">{{ number_format($progress['financial'], 0) }}%</span>
                                            </div>
                                            <div class="h-2 overflow-hidden rounded-full bg-slate-200 dark:bg-slate-800"><div class="h-full rounded-full bg-gradient-to-r from-emerald-500 to-teal-300 transition-all duration-700" style="width: {{ $progress['financial'] }}%"></div></div>
                                        ...
                                        <div>
                                            <div class="mb-2 flex items-center justify-between text-[11px] font-bold uppercase tracking-wider">
                                                <span class="flex items-center gap-2 text-fuchsia-600 dark:text-fuchsia-300"><span class="h-2 w-2 rounded-full bg-fuchsia-500 dark:bg-fuchsia-400"></span>Compétences mutualisées</span>
                                                <span class="text-slate-900 dark:text-white">{{ number_format($progress['human'], 0) }}%</span>
                                            </div>
                                            <div class="h-2 overflow-hidden rounded-full bg-slate-200 dark:bg-slate-800"><div class="h-full rounded-full bg-gradient-to-r from-fuchsia-500 to-indigo-300 transition-all duration-700" style="width: {{ $progress['human'] }}%"></div></div>
                                ...
                                <div class="flex items-center justify-between border-t border-slate-200/80 dark:border-white/10 bg-slate-50/50 dark:bg-black/10 px-6 py-4">
                                    <div class="flex min-w-0 items-center gap-3">
                                        ...
                                        <div class="min-w-0">
                                            <div class="flex items-center gap-1.5 truncate text-xs font-bold text-slate-800 dark:text-slate-200">{{ $project->user?->name ?? 'Porteur RIGO' }} @if($profile?->is_verified)<svg class="h-3.5 w-3.5 shrink-0 text-sky-500 dark:text-sky-400" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 1 0 0-16 8 8 0 0 0 0 16Zm3.857-9.857a.75.75 0 0 0-1.06-1.06L9 10.879 7.203 9.08a.75.75 0 1 0-1.06 1.061l2.327 2.327a.75.75 0 0 0 1.06 0l4.327-4.326Z" clip-rule="evenodd" /></svg>@endif</div>
                ...
                @else
                    <x-ui.empty-state
                        class="bg-white/50 dark:bg-white/[0.03]"
            ...
            @if($projects->hasPages())
                <div class="mt-10 border-t border-slate-200 dark:border-white/10 pt-6 [&_nav]:flex [&_nav]:items-center [&_nav]:justify-between [&_nav_span]:border-slate-200 dark:[&_nav_span]:border-white/10 [&_nav_span]:bg-slate-100 dark:[&_nav_span]:bg-white/[0.05] [&_nav_span]:text-slate-500 [&_nav_a]:border-slate-200 dark:[&_nav_a]:border-white/10 [&_nav_a]:bg-slate-100 dark:[&_nav_a]:bg-white/[0.05] [&_nav_a]:text-slate-600 dark:[&_nav_a]:text-slate-300 [&_nav_a]:transition [&_nav_a:hover]:border-indigo-300 dark:[&_nav_a:hover]:border-indigo-400/40 [&_nav_a:hover]:bg-indigo-50 dark:[&_nav_a:hover]:bg-indigo-400/10">
                    {{ $projects->links() }}
                </div>
            @endif
        </section>
