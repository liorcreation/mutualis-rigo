<div class="fixed right-4 top-4 z-[60] lg:right-8 lg:top-6" wire:poll.15s>
    <div class="relative">
        <button wire:click="toggle" type="button" class="relative flex h-11 w-11 items-center justify-center rounded-2xl border border-white/10 bg-slate-950/85 text-slate-300 shadow-xl shadow-black/20 backdrop-blur-xl transition hover:border-indigo-400/40 hover:bg-indigo-500/15 hover:text-white" aria-label="Notifications" aria-expanded="{{ $open ? 'true' : 'false' }}">
            ...
            @if($unreadCount > 0)<span class="absolute -right-1 -top-1 flex min-h-5 min-w-5 items-center justify-center rounded-full border-2 border-slate-950 bg-rose-500 px-1 text-[9px] font-black text-white">{{ $unreadCount > 99 ? '99+' : $unreadCount }}</span>@endif
        </button>
        ...
        @if($open)
            <div class="absolute right-0 mt-3 w-[min(22rem,calc(100vw-2rem))] overflow-hidden rounded-3xl border border-white/10 bg-slate-900/95 shadow-2xl shadow-black/40 backdrop-blur-2xl">
                ...
                <div class="max-h-[24rem] divide-y divide-white/10 overflow-y-auto">
                    ...
                    @empty
                        <div class="px-5 py-12 text-center"><div class="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-white/[0.05] text-slate-500"><svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.6"><path stroke-linecap="round" stroke-linejoin="round" d="M15 17H9m9-3V9a6 6 0 0 0-12 0v5l-2 2h16l-2-2" /></svg></div><p class="mt-4 text-xs font-bold text-slate-400">Aucune notification</p><p class="mt-1 text-[10px] text-slate-600">Vous êtes à jour.</p></div>
                    @endforelse
                </div>
            </div>
        @endif
