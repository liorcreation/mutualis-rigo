<div class="min-h-screen bg-slate-950 text-slate-100">
    <div class="mx-auto flex min-h-[calc(100vh-8rem)] max-w-5xl flex-col px-4 py-6 sm:px-6 lg:py-10">
        <div class="mb-5 flex items-center gap-3">
            <a href="{{ route('projects.show', $project) }}" wire:navigate class="rounded-xl border border-white/10 px-3 py-2 text-xs font-bold text-slate-400 transition hover:bg-white/10 hover:text-white">← Projet</a>
            <div class="min-w-0">
                <p class="truncate text-sm font-black text-white">{{ $project->titre }}</p>
        ...
        <div class="flex min-h-0 flex-1 flex-col overflow-hidden rounded-3xl border border-white/10 bg-white/[0.045] shadow-2xl shadow-black/20 backdrop-blur-2xl">
            <div class="flex items-center gap-3 border-b border-white/10 bg-white/[0.04] px-5 py-4 sm:px-7">
                ...
                <div>
                    <p class="text-sm font-black text-white">{{ $participant->name }}</p>
                    <p class="mt-1 flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-emerald-300"><span class="h-1.5 w-1.5 rounded-full bg-emerald-400"></span> Espace sécurisé</p>
                </div>
            </div>
                ...
                @forelse($messages as $message)
                    ...
                    <div wire:key="message-{{ $message->id }}" class="flex {{ $mine ? 'justify-end' : 'justify-start' }}">
                        <div class="max-w-[85%] sm:max-w-[70%]">
                            ...
                            <p class="mt-1 px-1 text-[10px] text-slate-600 {{ $mine ? 'text-right' : '' }}">
            ...
            <form wire:submit="send" class="border-t border-white/10 bg-black/10 p-4 sm:p-5">
                <div class="flex items-end gap-3">
                    <label class="flex-1">
                        <span class="sr-only">Votre message</span>
                        <textarea wire:model.live="content" rows="2" placeholder="Écrire un message..." class="w-full resize-none rounded-2xl border border-white/10 bg-slate-950/70 px-4 py-3 text-sm text-white outline-none transition placeholder:text-slate-600 focus:border-indigo-400/60 focus:ring-4 focus:ring-indigo-500/10 @error('content') border-rose-400/70 @enderror"></textarea>
                    </label>
                    <label class="flex h-11 w-11 shrink-0 cursor-pointer items-center justify-center rounded-xl border border-white/10 text-slate-400 transition hover:border-indigo-400/40 hover:bg-indigo-400/10 hover:text-indigo-300" title="Joindre un document">
                ...
                @if($attachment)
                    <p class="mt-2 text-xs text-indigo-300">Fichier prêt : {{ $attachment->getClientOriginalName() }}</p>
                @endif
                @error('attachment')
