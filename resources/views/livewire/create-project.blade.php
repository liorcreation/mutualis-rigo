        ...
        <div class="mt-8 grid gap-8 lg:grid-cols-[0.8fr_1.4fr] lg:items-start">
            <div class="lg:sticky lg:top-28">
                ...
                <div class="mt-8 space-y-3">
                    <div class="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.04] p-4"><span class="flex h-9 w-9 items-center justify-center rounded-xl bg-indigo-400/10 text-indigo-300">01</span><span class="text-xs font-bold text-slate-300">Présentez clairement votre idée</span></div>
                    <div class="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.04] p-4"><span class="flex h-9 w-9 items-center justify-center rounded-xl bg-fuchsia-400/10 text-fuchsia-300">02</span><span class="text-xs font-bold text-slate-300">Ciblez les ressources utiles</span></div>
                    <div class="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.04] p-4"><span class="flex h-9 w-9 items-center justify-center rounded-xl bg-emerald-400/10 text-emerald-300">03</span><span class="text-xs font-bold text-slate-300">Suivez la mutualisation</span></div>
                </div>
            </div>
            ...
            <form wire:submit="save" class="relative overflow-hidden rounded-3xl border border-white/10 bg-white/[0.055] p-5 shadow-2xl shadow-black/30 backdrop-blur-2xl sm:p-8">
                ...
                <div class="mb-7 flex items-center justify-between border-b border-white/10 pb-5"><div><h2 class="text-lg font-black text-white">Fiche projet</h2><p class="mt-1 text-xs text-slate-500">Les champs marqués d’un * sont obligatoires.</p></div><span class="rounded-full border border-amber-300/20 bg-amber-400/10 px-3 py-1 text-[10px] font-bold text-amber-200">EN ÉTUDE</span></div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="rounded-2xl border border-fuchsia-400/15 bg-fuchsia-400/[0.04] p-4 sm:p-5">
                        <div class="mb-4 flex items-center justify-between"><div class="flex items-center gap-3"><span class="flex h-9 w-9 items-center justify-center rounded-xl bg-fuchsia-400/10 text-fuchsia-300">✦</span><div><h3 class="text-sm font-black text-fuchsia-100">Compétences recherchées</h3><p class="text-[11px] text-fuchsia-200/50">Ajoutez les rôles et niveaux nécessaires.</p></div></div><button wire:click="addCompetence" type="button" class="rounded-xl border border-fuchsia-300/20 px-3 py-2 text-[10px] font-bold text-fuchsia-200 transition hover:bg-fuchsia-400/10">+ Ajouter</button></div>
                        <div class="space-y-3">
                            @foreach($competences as $index => $competence)
                                <div wire:key="competence-{{ $index }}" class="grid grid-cols-[1fr_1fr_auto] gap-2">
                                    <div><input wire:model.live.debounce.300ms="competences.{{ $index }}.role" type="text" placeholder="Rôle / profil" class="w-full rounded-xl border border-white/10 bg-slate-950/70 px-3 py-3 text-xs text-white outline-none transition placeholder:text-slate-600 focus:border-fuchsia-400/60 @error('competences.'.$index.'.role') border-rose-400/70 @enderror">@error('competences.'.$index.'.role')<span class="mt-1 block text-[10px] text-rose-400">{{ $message }}</span>@enderror</div>
                                    <div><input wire:model.live.debounce.300ms="competences.{{ $index }}.niveau" type="text" placeholder="Niveau (ex. senior)" class="w-full rounded-xl border border-white/10 bg-slate-950/70 px-3 py-3 text-xs text-white outline-none transition placeholder:text-slate-600 focus:border-fuchsia-400/60 @error('competences.'.$index.'.niveau') border-rose-400/70 @enderror"></div>
                    ...
                    <div class="rounded-2xl border border-amber-400/15 bg-amber-400/[0.04] p-4 sm:p-5">
                        <div class="mb-4 flex items-center justify-between"><div class="flex items-center gap-3"><span class="flex h-9 w-9 items-center justify-center rounded-xl bg-amber-400/10 text-amber-300">▦</span><div><h3 class="text-sm font-black text-amber-100">Matériel requis</h3><p class="text-[11px] text-amber-200/50">Listez les équipements qui peuvent être mutualisés.</p></div></div><button wire:click="addMateriel" type="button" class="rounded-xl border border-amber-300/20 px-3 py-2 text-[10px] font-bold text-amber-200 transition hover:bg-amber-400/10">+ Ajouter</button></div>
                        <div class="space-y-3">
                            @foreach($materiels as $index => $materiel)
                                <div wire:key="materiel-{{ $index }}" class="flex gap-2"><input wire:model.live.debounce.300ms="materiels.{{ $index }}" type="text" placeholder="Ex. 2 ordinateurs portables" class="w-full rounded-xl border border-white/10 bg-slate-950/70 px-3 py-3 text-xs text-white outline-none transition placeholder:text-slate-600 focus:border-amber-400/60 @error('materiels.'.$index) border-rose-400/70 @enderror"><button wire:click="removeMateriel({{ $index }})" type="button" class="rounded-xl border border-white/10 px-3 text-slate-500 transition hover:border-rose-400/30 hover:text-rose-300" aria-label="Supprimer le matériel">×</button></div>
                            @endforeach
                        </div>
                    </div>
                </div>

                <button wire:loading.attr="disabled" type="submit" class="mt-8 flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-indigo-500 to-fuchsia-500 px-5 py-4 text-sm font-black text-white shadow-xl shadow-indigo-950/40 transition hover:from-indigo-400 hover:to-fuchsia-400 disabled:cursor-wait disabled:opacity-60"><span wire:loading.remove>Soumettre le projet <span aria-hidden="true">→</span></span><span wire:loading>Enregistrement...</span></button>
            </form>
        </div>
    </div>
</div>
